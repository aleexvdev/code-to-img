...
import { setBorder } from "@/store/features/editorSlice";
...
export const BorderSelector = () => {
  ...
  const borderSelector = useSelector(
    (state: RootState) => state.editorReducer.border
  );

  const handleBorderSelector = () => {
    dispatch(setBorder(!borderSelector));
  };
  ...
  return (
    ...
    >
      <div className="h-[30px] inline-flex relative pl-4 items-center select-none hyphens-auto break-words">
        <span className="text-[#CCCCCC] text-xs text-left">Border</span>
      </div>
          ...
          <div className="w-full flex items-center justify-between px-1 py-1 gap-2 bg-[#232323] rounded-lg">
            <button
              className={`w-1/2 flex items-center justify-center text-xs py-1 font-semibold ${
                !borderSelector ? "bg-[#404040]" : "bg-none"
              } rounded-md`}
              onClick={handleBorderSelector}
            >
              None
            </button>
            <button
